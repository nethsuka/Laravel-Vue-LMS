...
const no_filteredItems = computed(() => {
    ...
    return Arrays.resourcesPaymentDetails.filter(obj =>
        obj.student_name.toLowerCase().includes(lowerKeyword) ||
        obj.student_email.toLowerCase().includes(lowerKeyword)
    );
});
...
function unpaidCount() {
    return Arrays.resourcesPaymentDetails.filter(item => item.paid === 'no').length;
}

function paidCount() {
    return Arrays.resourcesPaymentDetails.filter(item => item.paid === 'yes').length;
}

function deleteSlipData() {
    const choice = confirm('Are you sure you want to proceed with the deletion ?');
    if (choice) {
        router.visit('/resources-payments/delete/expired', { method: 'delete' });
    }
}
...
function confirmDelete(id) {
    ...
    if (confirmed) {
        form1.slipID = id;
        form1.delete('/resources-payments/delete/unaccepted-payment', {
...
const confirmAction = (id) => {
    if (confirm('Are you sure you want to delete this record?')) {
        router.delete('/resources-payments', {
            data: {slipID: id},
            preserveScroll: true,
        });
    }
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8" :style="{ overflowY: 'auto', maxHeight: '85vh' }">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg px-6 py-3 mb-4">
                    <div class="grid grid-cols-3 gap-4">
                        ...
                        <div class="py-2 px-7">
                            <fwb-button color="red" class="ml-12" @click="deleteSlipData">Delete Expired Records</fwb-button>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg px-6 py-3">
                    ...
                    <fwb-tabs v-model="activeTab" class="p-4" variant="underline">
                        <fwb-tab name="first" title="New Payments">
                                            ...
                                            <template v-if="no_filteredItems.length > 0">
                                                <fwb-table-row v-for="(element, index) in no_filteredItems" :key="element.slip_id">
                                                    <template v-if="element.paid == 'no'">
                                                        ...
                                                        <fwb-table-cell class="max-w-48 truncate" :title="element.student_email">{{ element.student_email }}</fwb-table-cell>
                                                        <fwb-table-cell>
                                                            <div v-html="addbr(element.selected_resources)"></div>
                                                        </fwb-table-cell>
                                                        <fwb-table-cell>
                                                        ...
                                                        <fwb-table-cell class="flex justify-center">
                                                            <Link href="/resources-payments" method="patch" :data="{ slipID: element.slip_id }" as="button" type="button" class="text-white bg-gradient-to-r from-cyan-400 via-cyan-500 to-cyan-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-cyan-300 dark:focus:ring-cyan-800 font-medium rounded-lg text-sm px-4 py-1.5 text-center">
                                                                Accept
                                                            </Link>
                        ...
                        <fwb-tab name="Second" title="Accepted Payments">
                            <div class="relative overflow-x-auto shadow-md sm:rounded-lg p-4 bg-slate-200 pt-8">
                                <div class="flex flex-column sm:flex-row flex-wrap space-y-4 sm:space-y-0 items-center justify-between pb-4">
                                    <div>
                                            ...
                                            <template v-if="yes_filteredItems.length > 0">
                                                <fwb-table-row v-for="(element, index) in yes_filteredItems" :key="element.slip_id">
                                                    <template v-if="element.paid == 'yes'">
                                                        ...
                                                        <fwb-table-cell>
                                                            {{ element.expiry_date }}
                                                            <fwb-badge type="red" v-if="isExpired(element.expiry_date)" class="flex justify-center max-w-14">Expired</fwb-badge>
                                                        </fwb-table-cell>
                                                        <fwb-table-cell>
                                                            <fwb-button color="default" class="whitespace-nowrap px-3 py-1" @click="showModal(element)" outline>Slip Details</fwb-button>
                                                        </fwb-table-cell>
                                                        <fwb-table-cell>
                                                            <Link href="" @click="confirmAction(element.slip_id)" as="button" type="button" class="text-white bg-gradient-to-r from-red-400 via-red-500 to-red-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm px-4 py-1.5 text-center">
                                                                Delete
                                                            </Link>
                                                        </fwb-table-cell>
                                                    </template>
                    ...
                    <fwb-modal v-if="isShowModal" @close="closeModal" size="4xl">
                        ...
                        <template #body>
                            <p class="text-base leading-relaxed text-gray-500 dark:text-gray-400">
                                Student email - {{ tomodel.student_email }} | Name - {{
